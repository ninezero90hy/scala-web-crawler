      ...
      if (!SiteMap.tree.contains(url.toString) && url.toString.contains("wiprodigital.com") && !(url.toString.contains("facebook")) &&
        ...
        if (contentType.startsWith("text/html")) {
          ...
          val links: List[URL] = doc.getElementsByTag("a").asScala.map(e => e.attr("href")).filter(s =>
            urlValidator.isValid(s)).map(link => (new URL(link))).toList.distinct
          SiteMap.tree += Tuple2(url.toString, PageData(url, title, links))
          links.filter(url => (!SiteMap.tree.contains(url.toString) && url.toString.contains("wiprodigital.com") && !(url.toString.contains("facebook")) &&
            !(url.toString.contains("twitter")) && !(url.toString.contains("linkedin")))).map(a => self ! Visit(a))
        }
      }
  }
